...
document
  ...
  .addEventListener("input", debounce(showAlert, 2000));
  ...
  axios
    ...
    .then(function (response) {
      ...
      if (
        ...
        response.data.employee_details.error
      ) {
        console.log("bug2");
      }
      ...
      if (employees.length > 0) {
        employees.forEach(function (employee) {
          ...
          listHtml += `<a href="#" data-employee='${encodeURIComponent(
            employeeDetails
          )}' onclick="selectEmployee(this)"><li style="${listItemStyle}">
...
document
  .getElementById("employee_name")
  .addEventListener("input", debounce(searchEmployee, 1000));

// Function to handle when an employee is clicked
// Function to handle when an employee is clicked
function selectEmployee(element) {
  var employeeDetails = decodeURIComponent(
...
function toggleProjectTable() {
  ...
  if (
    projectSelectValue === "Indihood" ||
    projectSelectValue === "ONECLICKLCA"
  ) {
...
function toggleDesignationTable() {
  ...
  if (
    designationSelect === "intern" ||
    designationSelect === "jr_qa_engineer"
  ) {
...
document.getElementById("myForm").addEventListener("submit", function (event) {
  ...
  if (attendance == 0) {
    ...
    let confirmAttendance = confirm(
      "Attendance is 0. All the values will be set to 0. Are you sure you want to submit?"
    );
